...
impl App {
    ...
    pub fn update_torrent_state(&mut self, torrent_id: TorrentId, stats: TorrentStats) {
        if let Some(torrent) = self.torrents.get_mut(&torrent_id) {
            ...
            if let Some(pieces) = &stats.pieces.latest_completed {
                for piece_index in pieces.iter().cloned() {
                    ...
                    let file_range = torrent.storage.files_intersecting_piece(piece_index);
            ...
            if let Peers::Full(peers) = &mut torrent.peers {
                if is_seed {
                    peers.sort_by(|a, b| b.thruput.payload.up.rate.cmp(&a.thruput.payload.up.rate));
                } else {
                    peers.sort_by(|a, b| {
                        b.thruput
                            .payload
