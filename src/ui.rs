...
use bt_rust::{peer::session::ConnectionState, torrent::stats::Peers};
use tui::{
    backend::Backend,
...
pub fn draw(f: &mut Frame<impl Backend>, app: &mut App) {
    if let Some(torrent) = app.torrents.values().next() {
        ...
        // - first is split vertically between torrent info throughput rates
...

        {
            ...
            draw_throughout(f, torrent, chunks[1]);
        }
...
pub fn draw_metadata(f: &mut Frame<impl Backend>, torrent: &Torrent, area: Rect) {
    // TODO: avoid clones...
    let text = vec![
        create_key_value_span("Name: ", torrent.name.clone()),
        create_key_value_span(
            "Path: ",
            torrent
                .storage
                .download_dir
        ...
        create_key_value_span("Info hash: ", torrent.info_hash.clone()),
        create_key_value_span("Size: ", Unit::new(torrent.download_len).to_string()),
        create_key_value_span(
            "Piece len: ",
            Unit::new(torrent.piece_len as u64).to_string(),
        ),
        create_key_value_span("Elapsed: ", format!("{} s", torrent.run_duration.as_secs())),
        create_key_value_span(
            "Pieces: ",
            format!(
                "{}/{} (pending: {})",
                torrent.pieces.complete, torrent.pieces.total, torrent.pieces.pending,
            ),
        ),
        create_key_value_span("Connected peers: ", torrent.peers.len().to_string()),
    ];
...
pub fn draw_throughout(f: &mut Frame<impl Backend>, torrent: &Torrent, area: Rect) {
    // we're showing 4 throughput stats:
    // - download rate
    // - upload rate
    // - received protocol chatter rate
    // - send protocol chatter rate
    let chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints(
            [
    ...
    let mut draw_rate_sparkline = |title: &'static str, stats: &ThruputHistory, chunk| {
        let title = create_key_value_span(
            title,
            format!(
                "{}/s (peak: {})",
                Unit::new(stats.rate()),
    ...
    draw_rate_sparkline("Protocol up: ", &torrent.protocol.down, chunks[3]);
}

pub fn draw_progress_bar(f: &mut Frame<impl Backend>, torrent: &Torrent, area: Rect) {
    let downloaded_percent = (torrent.pieces.complete as f64 / torrent.pieces.total as f64) * 100.0;
...
pub fn draw_files(f: &mut Frame<impl Backend>, torrent: &Torrent, area: Rect) {
    let files: Vec<ListItem> = torrent
        ...
        .map(|file| {
            ...
            let downloaded_percent = (file.complete as f64 / file.info.len as f64) * 100.0;
        ...
        let peers: Vec<ListItem> = peers
            ...
            .map(|peer| {
                ...
                let header = {
                    ...
                    // peer id: we may not able to show it if we don't have
                    // it yet because peer is still connecting or if it's not
                    // valid utf8
                    if let Some(id) = peer.id.and_then(|id| String::from_utf8(id.to_vec()).ok()) {
                        buf += " :: ";
                        buf += &id;
                    } else if peer.state.connection == ConnectionState::Connecting {
                        buf += " :: connecting...";
                    }
                    ...
                    if peer.piece_count == torrent.pieces.total {
                        ...
                        let downloaded_percent =
                            (peer.piece_count as f64 / torrent.pieces.total as f64) * 100.0;
